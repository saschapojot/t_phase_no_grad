...
public:
    reader(const int &rowNum,const int &TInd, const unsigned long long& cellNum) {
        ...
        this->rowNum = rowNum;
        this->TRoot = "./version1Data/1d/funcquadratic/row" + std::to_string(rowNum) + "/";
        std::vector<std::string >sortedTDirs= scanTDirs(TRoot);
        this->TDir=TRoot+"/"+sortedTDirs[TInd]+"/";
        std::cout<<"selected TDir="<<TDir<<std::endl;
        try {
            this->UInOneFile = std::shared_ptr<double[]>(new double[version1Quadratic::loopMax],
                                                         std::default_delete<double[]>());
            this->x_inOneFile = std::shared_ptr<double[]>(new double[version1Quadratic::loopMax * cellNum],
                                                          std::default_delete<double[]>());
        }
