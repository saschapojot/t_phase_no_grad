...
public:
    reader(const int &rowNum, const int &TInd, const unsigned long long &cellNum) {
        ...
        this->rowNum = rowNum;
        this->TRoot = "./version1Data/1d/funcquadraticInsideL/row" + std::to_string(rowNum) + "/";
        std::vector<std::string> sortedTDirs = scanTDirs(TRoot);
        this->TDir = TRoot + "/" + sortedTDirs[TInd] + "/";
        std::cout << "selected TDir=" << TDir << std::endl;
        try {
            this->UInOneFile = std::shared_ptr<double[]>(new double[version1InsideLQuadratic::loopMax],
                                                         std::default_delete<double[]>());
            this->db_inOneFile = std::shared_ptr<double[]>(new double[version1InsideLQuadratic::loopMax],
                                                           std::default_delete<double[]>());
        }
...
public:
   ...
   unsigned long long  load_theta0A();
...
public:
    ...
    std::vector<std::string> theta0AFilesAll;
    ...
    std::vector<std::string> sorted_theta0AFilesAll;
    ...
    std::shared_ptr<double[]> theta0A_selected;
