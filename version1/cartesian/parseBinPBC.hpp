...
public:
    reader(const int &rowNum, const int &TInd, const unsigned long long &cellNum) {
        ...
        this->rowNum = rowNum;
        this->TRoot = "./version1Data/1d/funcquadraticCartesian/row" + std::to_string(rowNum) + "/";
        std::vector<std::string> sortedTDirs = scanTDirs(TRoot);
        this->TDir = TRoot + "/" + sortedTDirs[TInd] + "/";
        std::cout << "selected TDir=" << TDir << std::endl;
        try {
            this->UInOneFile = std::shared_ptr<double[]>(new double[version1CartesianQuadratic::loopMax],
                                                         std::default_delete<double[]>());
            this->db_inOneFile = std::shared_ptr<double[]>(new double[version1CartesianQuadratic::loopMax],
                                                           std::default_delete<double[]>());
        }
...
public:
    ...
    std::vector<std::string> x0AFilesAll;
    ...
    std::vector<std::string> sorted_x0AFilesAll;
    ...
    std::shared_ptr<double[]> x0A_selected;
